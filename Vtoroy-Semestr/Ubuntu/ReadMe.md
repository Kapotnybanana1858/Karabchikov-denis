...
Сервер посредник который использует мульти протокол для перердачи данных между устройствами(у мульти протокола правила передачи данных)
...
Командой connection = pika.BlockingConnection(pika.ConnectionParameters('localhost'))
...
<br>контейнер - тип, позволяющий инкапсулировать в себе объекты других типов
